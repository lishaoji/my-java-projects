...
public class MapServer {
    ...
    public static Map<String, Object> getMapRaster(Map<String, Double> params, OutputStream os) {
        ...
        double root_width = (ROOT_LRLON - ROOT_ULLON) * 288200 / 256;
        int depth = (int) Math.ceil(Math.log(root_width / dpp) / Math.log(2));
        depth = Math.min(depth, 7);
        QuadTree tree = new QuadTree(ROOT_ULLON,ROOT_ULLAT,ROOT_LRLON,ROOT_LRLAT, 7);
        // To store the Nodes that will be used for rastering (in order)
        ArrayList<Node> nodeArray = tree.traversal(depth, ullon, ullat, lrlon, lrlat);
        // Sort the Nodes according to their coordinate
        Collections.sort(nodeArray);
        /* Put parameters into the Map returned */
        rasteredImageParams.put("raster_ul_lon", nodeArray.get(0).getxTop());
        rasteredImageParams.put("raster_ul_lat", nodeArray.get(0).getyTop());
        rasteredImageParams.put("raster_lr_lon", nodeArray.get(nodeArray.size() - 1).getxBottom());
        rasteredImageParams.put("raster_lr_lat", nodeArray.get(nodeArray.size() - 1).getyBottom());
        double actual_dpp_x = (nodeArray.get(0).getxBottom() - nodeArray.get(0).getxTop()) / 256;
        double actual_dpp_y = (nodeArray.get(0).getyTop() - nodeArray.get(0).getyBottom()) / 256;
//        double rastered_width = Math.round((nodeArray.get(nodeArray.size() - 1).getxBottom()
//                - nodeArray.get(0).getxTop()) / actual_dpp_x);
//        double rastered_height = Math.round((nodeArray.get(0).getyTop()
//                - nodeArray.get(nodeArray.size() - 1).getyBottom()) / actual_dpp_y);
        int rastered_width = (int) Math.round((nodeArray.get(nodeArray.size() - 1).getxBottom()
                - nodeArray.get(0).getxTop()) / actual_dpp_x);
        int rastered_height = (int) Math.round((nodeArray.get(0).getyTop()
                - nodeArray.get(nodeArray.size() - 1).getyBottom()) / actual_dpp_y);
        rasteredImageParams.put("raster_width", rastered_width);
        rasteredImageParams.put("raster_height", rastered_height);
        rasteredImageParams.put("depth", depth);
        rasteredImageParams.put("query_success", true);

        /* Create a BufferedImage and write it to OutputStream */
        BufferedImage result = new BufferedImage(rastered_width,
                rastered_height, BufferedImage.TYPE_3BYTE_BGR);
        Graphics g = result.getGraphics();
        int x = 0;
        int y = 0;
        try {
            for (Node node : nodeArray) {
                BufferedImage bi = ImageIO.read(new File(IMG_ROOT + node.getTheFileName() + ".png"));
