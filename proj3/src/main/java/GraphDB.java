...
public class GraphDB {
    Map<String, TrieNode> nodeMap = new HashMap<>();
    Map<Integer, Set<TrieNode>> connections = new HashMap<>();
    int count = 0;
     ...
     */
    private void clean() {
        Map<String, TrieNode> connected = new HashMap<>();
        for (Set<TrieNode> s : connections.values()) {
            for (TrieNode tn : s) {
                connected.put(tn.ID, tn);
            }
        }
        nodeMap = connected;
    }
